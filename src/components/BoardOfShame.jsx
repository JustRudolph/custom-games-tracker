...
export default function BoardOfShame({ entries, players, canWrite, onCreate, onDelete, fullPage = false, onBack }) {
  ...
  return <main className={fullPage ? "board-shame-page" : "board-shame-section"}>
    ...
    {isOpen && <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setIsOpen(false)}><section className="panel board-shame-modal" role="dialog" aria-modal="true"><div className="modal-head"><div><p className="eyebrow">ADMIN ENTRY</p><h2>Add to the board</h2></div><button className="modal-close" type="button" onClick={() => setIsOpen(false)} aria-label="Close">×</button></div><form onSubmit={save} onPaste={handlePaste}><label>Player<select className="board-shame-player-select" required value={playerId} onChange={(event) => setPlayerId(event.target.value)}><option value="">Select player</option>{players.map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</select></label><label>Reason<textarea required maxLength={1000} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="What happened?" /></label><label>Image (optional)<input className="board-shame-file-input" type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => readImage(event.target.files?.[0])} /><span className="board-shame-paste-hint">Paste an image anywhere in this form, or choose a file.</span></label>{image && <div className="board-shame-upload-wrap"><img className="board-shame-upload-preview" src={image} alt="Preview" /><button className="ghost-btn" type="button" onClick={() => setImage("")}>Remove image</button></div>}{error && <div className="login-error" role="alert">{error}</div>}<button className="primary-btn" type="submit" disabled={isSaving}>{isSaving ? "Saving..." : "Add to board"} <span>-&gt;</span></button></form></section></div>}
