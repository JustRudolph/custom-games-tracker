...
const emptyForm = (initialTeams) => ({
  id: initialTeams?.id,
  date: initialTeams?.date || today(),
  blue: fillTeam(initialTeams?.blue),
  red: fillTeam(initialTeams?.red),
  winner: initialTeams?.winner || "",
  matchType: initialTeams?.matchType || "manual",
  status: initialTeams?.status || "complete",
  notes: initialTeams?.notes || "",
  statsImage: initialTeams?.statsImage || "",
});
...
function TeamEditor({
  ...
  return (
    <div
      className={"team-editor " + team + (isActive ? " active" : " inactive")}
    >
      <div className="team-title">
        <span className="team-dot" />
        {team.toUpperCase()} TEAM <span>{players.length}/5</span>
      </div>
      <div className="player-entry-head">
        <span>Player</span>
        <span>Role</span>
        <span>Champion</span>
        <span>K / D / A</span>
      ...
      {players.map((player, index) => (
        ...
        >
          <input
            ...
            onKeyDown={(event) => selectFirstMatchingPlayer(event, index)}
          />
          <select
            required
            value={player.role}
            onChange={(event) => selectRole(index, event.target.value)}
          >
          ...
          <ChampionPicker
            ...
            isLoading={isChampionLibraryLoading}
            onChange={(champion) => update(index, "champion", champion)}
          />
          <div className="kda-inputs">
            <input
              ...
              placeholder="K"
              value={player.kills}
              onChange={(event) => update(index, "kills", event.target.value)}
            />
            <input
              type="number"
              min="0"
              ...
              placeholder="D"
              value={player.deaths}
              onChange={(event) => update(index, "deaths", event.target.value)}
            />
            <input
              type="number"
              min="0"
              ...
              onChange={(event) => update(index, "assists", event.target.value)}
            />
          </div>
        </div>
      ))}
...
export default function MatchForm({
  ...
  function readStatsImage(file) {
    ...
    reader.onerror = () => setError("Could not read the stats image.");
  ...
  useEffect(() => {
    const closeOnEscape = (event) => {
      if (event.key === "Escape" && !isCloseConfirmationOpen)
  ...
  async function submit(event, status = "complete") {
    ...
    if (names.some((name) => !name))
      return setError("Select all ten players before saving.");
    if (new Set(names).size !== names.length)
      return setError("Each player can only appear once in a match.");
    for (const [teamName, team] of [
      ["Blue", form.blue],
      ["Red", form.red],
    ]) {
      if (new Set(team.map((player) => player.role)).size !== ROLES.length)
        return setError(`${teamName} team must use each role once.`);
      if (requiresCompleteDetails && team.some((player) => !player.champion))
        return setError(
          `Select a champion for every ${teamName.toLowerCase()} team player.`,
        );
      if (
        requiresCompleteDetails &&
        team.some((player) =>
          [player.kills, player.deaths, player.assists].some(
            (value) => value === "",
          ),
        )
      )
        return setError(
          `Enter K/D/A for every ${teamName.toLowerCase()} team player.`,
        );
    }
    if (requiresCompleteDetails && !form.winner)
      return setError("Select the winning team before submitting the match.");
    const toPlayer = (player) => ({
      ...player,
      name: player.name.trim(),
      kda: [player.kills, player.deaths, player.assists].every(
        (value) => value !== "",
      )
        ? [player.kills, player.deaths, player.assists].join("/")
        : "-",
    });
    ...
    try {
      await onSaveMatch({
        id: form.id,
        date: form.date,
        blue: form.blue.map(toPlayer),
        red: form.red.map(toPlayer),
        winner: form.winner,
        matchType: form.matchType,
        status,
        notes: form.notes.trim(),
        statsImage: form.statsImage,
      });
      onClose();
    } catch (saveError) {
  ...
  return (
    <div
      ...
      onMouseDown={() => setIsCloseConfirmationOpen(true)}
    >
      <div
        className="panel entry-panel modal-panel"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="panel-head">
          <div>
            ...
            <h2>
              {form.status === "draft" ? "Complete custom" : "Log a custom"}
            </h2>
          </div>
          <button
        ...
        <fieldset className="match-type-field">
          <legend>Custom type</legend>
          <div className="match-type-options">
            {[
              ["manual", "Summoner's Rift"],
              ["spin", "Spin wheel"],
            ].map(([type, label]) => (
              <label
                className={form.matchType === type ? "selected" : ""}
                key={type}
              >
                <input
                  type="radio"
        ...
        >
          ...
          {!canWrite && (
            <div className="guest-review-note" role="note">
              <strong>Admin review required</strong>
              <span>
                Your custom will only appear in match history and player
                statistics after an admin approves it.
              </span>
            </div>
          )}
          {championLibraryError && (
            <div className="login-error">
              Champion library unavailable: {championLibraryError}
            </div>
          )}
          <div className="mobile-team-tabs" role="tablist" aria-label="Teams">
            {["blue", "red"].map((team) => (
              <button
              ...
              >
                ...
                {team[0].toUpperCase() + team.slice(1)} team
              </button>
            ))}
          </div>
          <div className="team-editors">
            <TeamEditor
              ...
              onChange={(blue) => setForm({ ...form, blue })}
            />
            <TeamEditor
              team="red"
              players={form.red}
          ...
          <div className="match-form-actions">
            ...
            <button
              ...
              type="submit"
              disabled={isSubmitting || Boolean(championLibraryError)}
            >
              {isSubmitting
                ? canWrite
                  ? "Saving match..."
      ...
      {isCloseConfirmationOpen && (
        <DiscardConfirmation
          ...
          message="The teams, champions, and K/D/A values you entered will be lost."
