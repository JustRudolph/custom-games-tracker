...
export default function MatchForm({ onSaveMatch, onClose, playerNames, champions, isChampionLibraryLoading, championLibraryError, initialTeams }) {
  ...
  async function submit(event, status = "complete") {
    ...
    for (const [teamName, team] of [["Blue", form.blue], ["Red", form.red]]) {
      ...
      if (status === "complete" && team.some((player) => !player.champion)) return setError(`Select a champion for every ${teamName.toLowerCase()} team player.`);
      if (status === "complete" && team.some((player) => [player.kills, player.deaths, player.assists].some((value) => value === ""))) return setError(`Enter K/D/A for every ${teamName.toLowerCase()} team player.`);
    }
    if (status === "complete" && !form.winner) return setError("Select the winning team before completing the match.");
  ...
  return <div className="modal-backdrop" onMouseDown={() => setIsCloseConfirmationOpen(true)}><div className="panel entry-panel modal-panel" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
    <div className="panel-head"><div><p className="eyebrow">{form.status === "draft" ? "MATCH DRAFT" : form.matchType === "spin" ? "SPIN WHEEL CUSTOM" : "NEW ENTRY"}</p><h2>{form.status === "draft" ? "Complete custom" : "Log a custom"}</h2></div><button className="modal-close" type="button" onClick={() => setIsCloseConfirmationOpen(true)}>x</button></div>
    <datalist id="player-options">{playerNames.map((name) => <option value={name} key={name} />)}</datalist>
    <fieldset className="match-type-field"><legend>Custom type</legend><div className="match-type-options">{[["manual", "Manual"], ["spin", "Spin wheel"]].map(([type, label]) => <label className={form.matchType === type ? "selected" : ""} key={type}><input type="radio" name="matchType" value={type} checked={form.matchType === type} onChange={(event) => setForm({ ...form, matchType: event.target.value })} /><span>{label}</span></label>)}</div></fieldset>
    <form onSubmit={(event) => submit(event, "complete")}><label>Date<input type="date" required value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /></label>{championLibraryError && <div className="login-error">Champion library unavailable: {championLibraryError}</div>}<div className="mobile-team-tabs" role="tablist" aria-label="Teams">{['blue', 'red'].map((team) => <button className={activeTeam === team ? 'active' : ''} type="button" role="tab" aria-selected={activeTeam === team} key={team} onClick={() => setActiveTeam(team)}><span className={'team-dot ' + team} />{team[0].toUpperCase() + team.slice(1)} team</button>)}</div><div className="team-editors"><TeamEditor team="blue" players={form.blue} champions={champions} isChampionLibraryLoading={isChampionLibraryLoading} isActive={activeTeam === 'blue'} onChange={(blue) => setForm({ ...form, blue })} /><TeamEditor team="red" players={form.red} champions={champions} isChampionLibraryLoading={isChampionLibraryLoading} isActive={activeTeam === 'red'} onChange={(red) => setForm({ ...form, red })} /></div><div className="winner-row"><span className="label">WINNER</span><div className="winner-options">{['blue', 'red'].map((team) => <label key={team}><input type="radio" name="winner" value={team} checked={form.winner === team} onChange={(event) => setForm({ ...form, winner: event.target.value })} /><span>{team[0].toUpperCase() + team.slice(1)} team</span></label>)}</div></div><label>Notes<input maxLength="2000" placeholder="Optional context" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} /></label>{error && <div className="login-error" role="alert">{error}</div>}<div className="match-form-actions"><button className="ghost-btn" type="button" disabled={isSubmitting} onClick={(event) => submit(event, "draft")}>{isSubmitting ? "Saving..." : "Save draft"}</button><button className="primary-btn" type="submit" disabled={isSubmitting || Boolean(championLibraryError)}>{isSubmitting ? "Saving match..." : form.id ? "Complete match" : "Save completed match"} <span>-&gt;</span></button></div></form>
