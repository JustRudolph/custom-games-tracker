...
export default function MatchForm({ onAddMatch, onClose, playerNames, champions, savedPlayers, isChampionLibraryLoading, championLibraryError, initialTeams }) {
  ...
  async function submit(event) {
    ...
    try {
      await onAddMatch({ date: form.date, blue: form.blue.map(toPlayer), red: form.red.map(toPlayer), winner: form.winner, matchType: form.matchType, notes: form.notes.trim() });
      onClose();
    } catch (saveError) {
