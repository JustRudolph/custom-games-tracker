        ...
        {drawPool !== null && (
          <div className="spin-custom-layout">
            ...
            <section className="spin-stage">
              ...
              {rolesComplete && <div className="spin-complete"><p className="eyebrow">TEAMS READY</p><h3>Roles are assigned</h3>{canWrite && <p className="spin-review-note">{isSavingDraft ? "Saving draft..." : savedDraft ? "Draft saved to match history." : "The draft could not be saved."}</p>}{draftError && <div className="login-error" role="alert">{draftError}</div>}{canWrite && draftError && <button className="ghost-btn spin-retry-draft" type="button" onClick={saveCompletedDraft}>Retry saving draft</button>}{canWrite ? <div className="spin-complete-actions"><button className="secondary-btn" type="button" onClick={copyTeams}>Copy teams</button><button className="primary-btn" type="button" disabled={!savedDraft} onClick={useTeams}>Complete draft <span>-&gt;</span></button></div> : <button className="primary-btn spin-copy-teams-button" type="button" onClick={copyTeams}>Copy teams</button>}{copyStatus && <span className="spin-copy-status" role="status">{copyStatus}</span>}</div>}
            </section>
          </div>
        )}
      </section>
