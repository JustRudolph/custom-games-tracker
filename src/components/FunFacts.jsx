...
export default function FunFacts({ facts, canWrite, onCreate, onUpdate, onDelete }) {
  ...
  return <>
    <section className="fun-facts-bar" aria-label="Diamond Dynasty fun fact">
      <span className="fun-facts-label">DYNASTY FACT</span>
      <span className="fun-facts-text">{currentFacts.length ? currentFacts[activeIndex % currentFacts.length].text : "Add the first Diamond Dynasty fun fact."}</span>
      <button type="button" className="fun-facts-all" onClick={() => { setError(""); setIsOpen(true); }}>All facts</button>
    ...
    {isOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setIsOpen(false); }}>
      <section className="panel fun-facts-modal" role="dialog" aria-modal="true" aria-labelledby="fun-facts-title">
        <div className="modal-head"><div><p className="eyebrow">DIAMOND DYNASTY</p><h2 id="fun-facts-title">Custom game fun facts</h2></div><button type="button" className="modal-close" onClick={() => setIsOpen(false)} aria-label="Close">×</button></div>
